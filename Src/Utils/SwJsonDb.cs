...
public class SwJsonDb
{
	...
	public bool TryGetDbArray(string path, out SwJsonDb[] res)
	{
		res = default;
		if(!TryGetPath(path, out var node)) return false;
		if(!TryAsArray(node, out var arr)) return false;
		res = [.. arr.Select(node => new SwJsonDb(node))];
		return true;
	}
	...
	public static JsonType GetType(JsonNode node)
	{
		...
		return kind switch
		{
			...
			System.Text.Json.JsonValueKind.String => JsonType.String,
			_ => throw new Exception($"Unexpected json kind '{kind}'"),
		};
	}
}
